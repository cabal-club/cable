...
effect is that the `recipient` defined in the post should have their
...
`blocked`, and sets `recipient = [blocked.public_key]`. When `issuer`
...
Note how `recipient` is the same for `post/block` and `post/unblock`. We
...
Consider the following from the perspective of the local user. If a post
undoes the effects of another post with `privacy 1 = local-only` then
the undoing post MUST also have its field `privacy` set to
`1 = local-only`.

For example, say the local user issued a `post/moderation` with
`action = 0 hide user` where with `recipient = hidden-user.public_key`
(call this post *post 1*).

`post/moderation` has a field `privacy`, and the local user set *post
1*'s `privacy` to `1 = local-only` for instance because they didn't want
to cause a conflict in hiding `hidden-user`. Some time later, they feel
like unhiding `hidden-user`, this is done with a `post/moderation` with
`action = 1 unhide user` (call this post *post 2*). *Post 2*'s field
`recipient` should refer to the same `recipient` as *post 1* (i,e,
`hidden-user.public_key`).

The result is that *post 2*'s action undoes the effects of *post 1*. If
these conditions hold true—the recipients overlap, a newer post undoes
the action encoded in an older post, the older post has
`privacy = 1 local-only`—then *post 2* MUST also have its field
`privacy` set to `1 = local-only`.
